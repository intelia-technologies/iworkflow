    ...
    "steps": [
        {
            ...
            "agents": [
                {
                    "id": "eng",
                    "prefer": ["cursor:flash", "codex", "gemini"],
                    "prompt": "From an engineering angle, answer concisely: {{params.goal}}",
                },
                {
                    "id": "dx",
                    "prefer": ["gemini", "cursor:flash", "codex"],
...
REVIEW: dict[str, Any] = {
    ...
    "output": {
        "gate": "{{steps.gate.value}}",
        "reviews": "{{steps.fan.value}}",
...
ADAPTIVE_REVIEW: dict[str, Any] = {
    ...
    "output": {
        "gate": "{{steps.gate.value}}",
        "reviews": "{{steps.fan.value}}",
        "supervision": "{{steps.supervise.value}}",
        "audit": "{{steps.audit.value}}",
    },
}
...
BRAINSTORM: dict[str, Any] = {
    ...
    "params": {"change_name": "feature-x", "user_input": ""},
    "schemas": {"proposal": PROPOSAL_SCHEMA, "decision": DECISION_SCHEMA},
    "steps": [
        {
            "id": "phase1_search",
            "kind": "agent",
        ...
        },
        {
            ...
            "instructions": {
                "gh": "gh pr create --title 'Brainstorm: {{params.change_name}}' --body 'Generated via iworkflow'"
            },
        ...
        },
        {
            ...
            "heartbeat_interval_s": 15,
            "when": {
                "path": "steps.phase5_dialogue_loop",
                "select": "stop_reason",
                "eq": "max_iterations",
            },
...
DESIGN_WORKFLOW: dict[str, Any] = {
    ...
    "artifacts": [{"path": ".iworkflow/recipes/{{params.workflow_name}}.json", "type": "file"}],
    "output": {
...
try:
    GLOBAL_RECIPE_DIR: Path | None = Path.home() / ".iworkflow" / "recipes"
except Exception:
...
def all_recipes(extra_dir: str | None = None) -> dict[str, dict[str, Any]]:
    """Built-in recipes overlaid with global user-wide recipes, overlaid with host-project recipes (host wins on name)."""
    recipes = dict(BUILTIN)
    if GLOBAL_RECIPE_DIR and GLOBAL_RECIPE_DIR.is_dir():
        recipes.update(_load_dir(str(GLOBAL_RECIPE_DIR)))
    recipes.update(_load_dir(extra_dir))
    return recipes
