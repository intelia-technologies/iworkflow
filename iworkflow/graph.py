...
_NODE_DEF_RE = re.compile(r"^\s*([A-Za-z_][A-Za-z0-9_]*)\s*(?:\[|\{|\(\()")
_SUBGRAPH_RE = re.compile(r"^\s*subgraph\s+([A-Za-z_][A-Za-z0-9_]*)\b")
_EDGE_RE = re.compile(r"^\s*([A-Za-z_][A-Za-z0-9_]*)\b.*?--[^>]*>\s*([A-Za-z_][A-Za-z0-9_]*)\b")
_CHAIN_EDGE_RE = re.compile(r"--[^>]*>\s*([A-Za-z_][A-Za-z0-9_]*)\b")


class MermaidValidationError(ValueError):
    """Raised when generated Mermaid is not safe to hand to Mermaid.js."""
...
def _node_id(value: Any) -> str:
    """Return a Mermaid-safe node id for arbitrary workflow/step ids.

    Mermaid v10 is much stricter than our recipe ids: hyphens, dots, slashes and
    other punctuation can parse as operators. Labels keep the original id; the
    internal graph id is sanitized and hash-suffixed when changed so collisions do
    not collapse nodes.
    """
    raw = str(value or "node")
    if _SAFE_ID.fullmatch(raw):
        return raw
    sanitized = re.sub(r"[^A-Za-z0-9_]", "_", raw).strip("_") or "node"
    if not re.match(r"[A-Za-z_]", sanitized):
        sanitized = f"n_{sanitized}"
    return f"{sanitized}_{_fnv1a_32(raw)}"


def validate_mermaid(code: str) -> None:
    """Validate the subset of Mermaid generated by this module.

    This is intentionally local and dependency-free: it catches our historical
    failure mode (unsafe node identifiers) before a browser/terminal tries to
    auto-render the fence. Mermaid.js remains the canonical renderer, but invalid
    generated syntax should fail fast with a line number and source line.
    """
    if not code.strip():
        raise MermaidValidationError("empty Mermaid document")
    lines = code.splitlines()
    if lines[0].strip() != "graph TD":
        raise MermaidValidationError("line 1: expected 'graph TD'")
    for lineno, raw_line in enumerate(lines[1:], 2):
        line = raw_line.strip()
        if not line or line == "end" or line.startswith("classDef ") or line.startswith("class "):
            # Dynamic dashboard code appends class/classDef lines after this source.
            continue
        if line.startswith("subgraph "):
            if not _SUBGRAPH_RE.match(raw_line):
                raise MermaidValidationError(f"line {lineno}: invalid subgraph id: {raw_line.strip()}")
            continue
        if "-->" in line:
            if not _EDGE_RE.match(raw_line):
                raise MermaidValidationError(f"line {lineno}: invalid edge syntax: {raw_line.strip()}")
            for node in _CHAIN_EDGE_RE.findall(raw_line):
                if not _SAFE_ID.fullmatch(node):
                    raise MermaidValidationError(f"line {lineno}: invalid edge target id {node!r}")
            continue
        if _NODE_DEF_RE.match(raw_line):
            continue
        raise MermaidValidationError(f"line {lineno}: unsupported generated Mermaid line: {raw_line.strip()}")
...
def render_agent(step_id: str, agent_spec: dict[str, Any],
    ...
    return node_def, node_id, node_id
...
def render_parallel(step_id: str, step_spec: dict[str, Any],
    ...
    lines = []
    group_id = _node_id(step_id)
    entry_id = _node_id(f"{step_id}_entry")
    exit_id = _node_id(f"{step_id}_exit")
    lines.append(f'    subgraph {group_id} ["parallel: {_esc(step_id)}"]')
    lines.append(f'        {entry_id}(( ))')
    lines.append(f'        {exit_id}(( ))')

    for i, a in enumerate(agents):
        a_id = a.get("id") or f"agent_{i}"
        full_a_id = _node_id(f"{step_id}_{a_id}")
        ...
        lines.append(f'        {entry_id} --> {full_a_id} --> {exit_id}')

    lines.append('    end')

    return "\n".join(lines), entry_id, exit_id
...
def render_pipeline(step_id: str, step_spec: dict[str, Any],
    ...
    lines = []
    group_id = _node_id(step_id)
    lines.append(f'    subgraph {group_id} ["pipeline: {_esc(step_id)} ({items})"]')
    ...
    for i, s in enumerate(stages):
        ...
        full_s_id = _node_id(f"{step_id}_{s_id}")
...
def render_loop(step_id: str, step_spec: dict[str, Any], ctx: dict[str, Any],
    ...
    group_id = _node_id(step_id)
    lines.append(
        f'    subgraph {group_id} ["loop: {_esc(step_id)} '
        f'(max_iterations: {_esc(max_iter)})"]'
    )

    cond_node = _node_id(f"{step_id}_cond")
...
def render_step(step: dict[str, Any], ctx: dict[str, Any],
    ...
    if kind == "agent":
        ...
        node_id = _node_id(step_id)
        return f'    {node_id}["{_text(step_id, ctx)} (command)"]', node_id, node_id
    else:
        node_id = _node_id(step_id)
        return f'    {node_id}["{_text(step_id, ctx)} (unknown: {_esc(kind)})"]', node_id, node_id
