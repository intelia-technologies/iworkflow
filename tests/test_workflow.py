...
def _scripted_runner(tmp_path, responder, run_id="wf"):
    ...
    runner = Runner(
        run_id,
        {"codex": p, "gemini": p, "claude": p},
        {"codex": 4, "gemini": 4, "claude": 4},
...
def test_agent_step_templates_prompt_and_output(tmp_path):
    ...
    spec = {
        "steps": [
            {"id": "a", "kind": "agent", "prefer": ["codex"], "prompt": "solve {{params.q}}"}
        ],
...
def test_checkpoint_resume_from_valid_output_file(tmp_path):
    ...
    steps = json.loads(
        (tmp_path / "runs" / "checkpoint-resume" / "wf-steps.json").read_text(encoding="utf-8")
    )
...
def test_agent_when_executes_when_true_and_skips_when_false_without_provider(tmp_path):
    ...
    events = [json.loads(line) for line in events_path.read_text(encoding="utf-8").splitlines()]
    assert any(
        e["event"] == "skipped" and e["label"] == "audit" and e["kind"] == "agent" for e in events
    )
...
def test_command_when_true_executes_and_false_skips(tmp_path):
    ...
    steps = json.loads(
        (tmp_path / "runs" / "conditional-command" / "wf-steps.json").read_text(encoding="utf-8")
    )
...
def test_compound_when_over_multiple_steps_skips_when_false(tmp_path):
    ...
    steps = json.loads(
        (tmp_path / "runs" / "conditional-compound" / "wf-steps.json").read_text(encoding="utf-8")
    )
...
def test_skipped_step_resume_uses_journal_without_reevaluating(tmp_path):
    ...
    steps = json.loads(
        (tmp_path / "runs" / "conditional-resume" / "wf-steps.json").read_text(encoding="utf-8")
    )
    assert steps["lint"] == {"skipped": True, "ok": True, "kind": "agent"}
    events = [
        json.loads(line)
        for line in (tmp_path / "runs" / "conditional-resume" / "events.jsonl")
        ...
        "steps": [
            {
                ...
                "body": [{"id": "b", "kind": "agent", "prefer": ["codex"], "prompt": "work"}],
            }
        ]
    }
    _run(run_spec(runner, spec))
        ...
        "steps": [
            {
                ...
                "until": {
                    "agent": {
                        "prompt": "complete? {{loop.collected}}",
                        "stop_when": "STOP",
                        "prefer": ["codex"],
                    }
                },
        ...
        "steps": [
            {
                ...
                "max_iterations": 3,
                "until": {
                    "agent": {"prompt": "complete?", "stop_when": "STOP", "prefer": ["codex"]}
                },
                "collect": {"from": "find", "path": "findings", "dedup_by": "title"},
                "body": [
                    {
...
def test_host_recipe_dir_discovered(tmp_path):
    ...
    rdir.mkdir()
    (rdir / "custom.json").write_text(
        json.dumps(
            {
                "name": "custom",
                "description": "host recipe",
...
def test_global_recipe_dir_discovered(tmp_path, monkeypatch):
    import json
    from iworkflow import recipes

    fake_home = tmp_path / "fake_home"
    fake_home.mkdir()
    monkeypatch.setattr(recipes.Path, "home", lambda: fake_home)
    monkeypatch.setattr(recipes, "GLOBAL_RECIPE_DIR", fake_home / ".iworkflow" / "recipes")

    global_rdir = fake_home / ".iworkflow" / "recipes"
    global_rdir.mkdir(parents=True)
    (global_rdir / "global_custom.json").write_text(
        json.dumps(
            {
                "name": "global_custom",
                "description": "global user recipe",
                "steps": [{"id": "a", "kind": "agent", "prefer": ["codex"], "prompt": "hi"}],
            }
        )
    )
...
def test_tools_injection_rejected_by_default(tmp_path):
    ...
    spec = {
        "steps": [
            {"id": "a", "kind": "agent", "prefer": ["codex"], "tools": ["postgres"], "prompt": "go"}
        ]
    }
    with pytest.raises(WorkflowLimitError):
        _run(run_spec(runner, spec))
        ...
        "steps": [
            {
                ...
                "kind": "loop",
                "max_iterations": 3,
                "until": {"vote": {"prompt": "?", "count": 3, "threshold": 0, "stop_when": "STOP"}},
        ...
        "steps": [
            {
                ...
                "kind": "loop",
                "max_iterations": 3,
                "until": {"vote": {"prompt": "?", "count": 3, "threshold": 4, "stop_when": "STOP"}},
                "collect": {"from": "f", "dedup_by": "title"},
                "body": [{"id": "f", "kind": "agent", "prompt": "p"}],
            }
        ]
        ...
        "steps": [
            {
                "id": "fan",
                "kind": "parallel",
                "agents": [{"id": f"a{i}", "prompt": "p"} for i in range(3)],
            }
        ]
    }
    with pytest.raises(WorkflowLimitError):
...
def test_loop_nesting_depth_capped():
    ...
    mid = {"id": "L2", "kind": "loop", "max_iterations": 2, "until": {"times": 1}, "body": [inner]}
    outer = {"id": "L1", "kind": "loop", "max_iterations": 2, "until": {"times": 1}, "body": [mid]}
    with pytest.raises(WorkflowLimitError):
        ...
        "steps": [
            {
                ...
                "max_iterations": 10,
                "until": {"times": 10},
                "body": [{"id": "b", "kind": "agent", "prefer": ["codex"], "prompt": "tick"}],
            }
        ]
    }
    with pytest.raises(WorkflowLimitError):
...
def test_pipeline_items_capped(tmp_path):
    ...
    with pytest.raises(WorkflowLimitError):
        _run(run_spec(runner, spec, {"items": [1, 2, 3]}, limits=Limits(max_pipeline_items=2)))
...
def test_required_agent_exhaustion_fails_workflow(tmp_path):
    provider = FakeProvider("codex", limit_first_n=99)
    runner = Runner(
        "required-exhausted", {"codex": provider}, {"codex": 1}, journal_dir=str(tmp_path)
    )
    spec = {
        "steps": [{"id": "critical", "kind": "agent", "prefer": ["codex"], "prompt": "must work"}]
    }
...
def test_required_nested_agent_exhaustion_reports_full_label_and_timeout(tmp_path):
    provider = FakeProvider("codex", limit_first_n=99)
    runner = Runner(
        "nested-exhausted", {"codex": provider}, {"codex": 1}, journal_dir=str(tmp_path)
    )
    spec = {
        "steps": [
            {
...
def test_workflow_abort_kills_in_flight_subprocesses(tmp_path):
    ...
    spec = {
        "steps": [
            ...
            {"id": "step_b", "kind": "command", "command": [sys.executable, "-c", cmd_script]},
        ]
    }
...
def test_optional_agent_exhaustion_can_continue(tmp_path):
    provider = FakeProvider("codex", limit_first_n=99)
    runner = Runner(
        "optional-exhausted", {"codex": provider}, {"codex": 1}, journal_dir=str(tmp_path)
    )
    spec = {
        "steps": [
            {
...
def test_required_agent_schema_mismatch_fails_workflow(tmp_path):
    ...
    events = [
        json.loads(line)
        for line in (tmp_path / "runs" / "schema-required" / "events.jsonl")
        .read_text()
        .splitlines()
    ]
...
def test_agent_schema_mismatch_fails_over_to_next_provider(tmp_path):
    ...
    events = [
        json.loads(line)
        for line in (tmp_path / "runs" / "schema-failover" / "events.jsonl")
        .read_text()
        .splitlines()
    ]
...
def test_parallel_schema_mismatch_marks_subresult_failed(tmp_path):
        ...
        "steps": [
            {
                ...
                "kind": "parallel",
                "agents": [
                    {"id": "a", "prefer": ["codex"], "fallback": False,
                     "schema": schema, "prompt": "agent a"},
                    {"id": "b", "prefer": ["codex"], "fallback": False,
                     "schema": schema, "prompt": "agent b"},
                ],
            }
        ],
        "output": "{{steps.fan.ok}}",
    ...
    events = [
        json.loads(line)
        for line in (tmp_path / "runs" / "schema-parallel" / "events.jsonl")
        .read_text()
        .splitlines()
    ]
    mismatches = [event for event in events if event["event"] == "schema_mismatch"]
...
def test_write_agent_fails_when_touching_unallowed_path(tmp_path):
    ...
    class WrongWriter(Provider):
        async def run(
            self, prompt, *, schema=None, sandbox="read-only", cwd=None, toolset=None, model=None
        ):
    ...
    with pytest.raises(WorkflowError) as exc_info:
        _run(
            run_spec(
                ...
                spec,
                limits=Limits(
                    allow_tools=True, allowed_sandboxes=frozenset({"read-only", "write"})
                ),
            )
        )
...
def test_preflight_checks_uncommitted_changes(tmp_path):
    ...
    spec = {
        "name": "preflight_test",
        "execution": {"worktree": "new:branch-name", "git_clean_required": True},
        "steps": [{"id": "s1", "kind": "agent", "prefer": ["codex"], "prompt": "hello"}],
    }
